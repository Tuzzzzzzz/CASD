...
public class MyLinkedList<T> : IMyList<T>, IMyDeque<T>
{
    private Node<T>
        _first = null,
        _last = null;
    ...
    public MyLinkedList(IMyCollection<T> collection) => AddAll(collection);
    ...
    private class ListItr : ListIterator<T>
    {
        ...
        public ListItr(MyLinkedList<T> list, int index)
        {
            list.CheckPositionIndex(index);
            _list = list;
            //null is the flag of the end of this list
            _nextNode = index == list.Size() - 1 ? null : list.NodeAt(index);
            _nextIndex = index;
        }
    ...
    public void AddAll(int index, IMyCollection<T> collection)
    {
        ...
        int i = 0;
        for (var it = ListIterator(); it.HasNext(); it.Next())
        {
            if (i == index)
            {
                foreach (T el in collection) it.Add(el);
            }
            i++;
        }
    }
    ...
    public IMyList<T> SubList(int fromIndex, int toIndex)
    {
        ...
        if (toIndex < fromIndex) throw new Exception();
