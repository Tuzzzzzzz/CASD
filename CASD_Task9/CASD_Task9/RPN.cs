...
internal static class RPN
{
    ...
    private static HashSet<string> binaryPrefixOperations = new() {
        "log", "min", "max"
    };
    ...
    public static HashSet<string> BinaryPrefixOperations { get => binaryPrefixOperations; }
    ...
    public static MyVector<string> ParseTokens(string expression)
    {
        ...
        foreach (char c in expression)
        {
            ...
            else if (RightAssociativeInfixOperations.Contains(token) || LeftAssociativeInfixOperations.Contains(token))
            {
                if (RightAssociativeInfixOperations.Contains(token + s) || LeftAssociativeInfixOperations.Contains(token + s))
                {
                    tokens.Add(token + s);
                    token = "";
                }
            ...
            else if (RightAssociativeInfixOperations.Contains(s) || LeftAssociativeInfixOperations.Contains(s))
            {
                if (token != "")
                {
                    tokens.Add(token);
    ...
    public static bool CheckTokens(MyVector<string> tokens)
    {
        foreach (string token in tokens)
        {
            if (!(double.TryParse(token, out _)
                ...
                || BinaryPrefixOperations.Contains(token)
    ...
    public static MyVector<string> ConvertToRPN(MyVector<string> tokens)
    {
        ...
        foreach (string token in tokens) {
            if (double.TryParse(token, out _)
                ...
                || BinaryPrefixOperations.Contains(token)) stack.Push(token);//right-associative operations
            ...
            else if (token == ")")
            {
                ...
                if (stack.Peek() != "(")
                    throw new ArgumentException("Пропущена открывающая скобка");
                stack.Pop();
                if (!stack.IsEmpty() && (UnaryPrefixOperations.Contains(stack.Peek()) || BinaryPrefixOperations.Contains(stack.Peek())))
                    RPN.Add(stack.Pop());
            }
        }
        while (!stack.IsEmpty()) {
    ...
    public static double EvaluateFunction(string operation, params double[] args) => operation switch
    {
        ...
        "%" => Math.Floor(args[0]) == args[0] && Math.Floor(args[1]) == args[1]
               ? (int)args[0] % (int)args[1] : throw new ArgumentException("Числа должны быть целыми"),
        "//" => Math.Floor(args[0]) == args[0] && Math.Floor(args[1]) == args[1] 
                ? (int)args[0] / (int)args[1] : throw new ArgumentException("Числа должны быть целыми"),
